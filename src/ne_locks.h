...
NE_API void ne_lock_free(struct ne_lock *lock);

/* Like ne_lock_free; but free's the lock object itself too. */
NE_API void ne_lock_destroy(struct ne_lock *lock);
...
NE_API void ne_lockstore_register(ne_lock_store *store, ne_session *sess);

/* Destroy a lock store, free'ing any locks remaining inside. */
NE_API void ne_lockstore_destroy(ne_lock_store *store);
 ...
 *  - a valid depth
 */
NE_API void ne_lockstore_add(ne_lock_store *store, struct ne_lock *lock);

/* Remove given lock object from store: 'lock' MUST point to a lock
 * object which is known to be in the store. */
NE_API void ne_lockstore_remove(ne_lock_store *store, struct ne_lock *lock);
...
NE_API int ne_lock(ne_session *sess, struct ne_lock *lock);

/* Issue an UNLOCK request for the given lock */
NE_API int ne_unlock(ne_session *sess, const struct ne_lock *lock);
 ...
 * given by the server. */
NE_API int ne_lock_refresh(ne_session *sess, struct ne_lock *lock);
...
NE_API void ne_lock_using_resource(ne_request *req, const char *path, int depth);
...
NE_API void ne_lock_using_parent(ne_request *req, const char *path);
